    ...
    if (!empty($list_cats)) {
        foreach ($list_cats as $cat) {
            if (empty($cat['parentid'])) {
				...
				//if (nv_menu_bootstrap_check_current($cat['link'], $cat['active_type'])) {
                //    $classcurrent[] = 'active';
                //} elseif (!empty($submenu_active)) {
                //    $classcurrent[] = 'active';
                //}
				//$cat['class'] = empty($classcurrent) ? '' : ' class="' . (implode(' ', $classcurrent)) . '"';
...
function nv_menu_check_current($url, $type = 0)
{
	...
	//$client_info['selfurl'] == https://www.kenmei.honguyen.info/users
	// var_dump($url);die;
